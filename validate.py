...
def custom_card_check(args, card, pack_code, types_data):
...
def parse_commandline():
    argparser = argparse.ArgumentParser(description="Validate JSON in the cards repository.")
    argparser.add_argument("-f", "--fix_formatting", default=True, action="store_true", help="write suggested formatting changes to files")
    argparser.add_argument("-v", "--verbose", default=1, action="count", help="verbose mode")
...
def validate_card(args, card, card_schema, pack_code, types_data):
    global validation_errors

    try:
        verbose_print(args, "Validating card %s... " % card["code"], 2)
        jsonschema.validate(card, card_schema)
        custom_card_check(args, card, pack_code, types_data)
...
def validate_cards(args, packs_data, types_data):
    global validation_errors
    ...
    for p in packs_data:
        ...
        if pack_data:
            for card in pack_data:
                validate_card(args, card, CARD_SCHEMA, p["code"], types_data)
...
def main():
    ...
    if packs and types:
        validate_cards(args, packs, types)
